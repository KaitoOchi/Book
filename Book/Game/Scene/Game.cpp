...
void Game::LevelDesign()
{
	...
	m_levelRender.Init("Assets/level3D/level0_1.tkl", [&](LevelObjectData& objData){
		...
		if (objData.ForwardMatchName(L"Normal") == true) {
			...
			// 自身の属性を教える
			enemyNormal->SetEnemyType(Enemy::TYPE_NORMAL);
			// 座標・回転・スケールを教える
		...
		if (objData.ForwardMatchName(L"Charge") == true) {
			...
			// 自身の属性を教える
			enemyCharge->SetEnemyType(Enemy::TYPE_CHARGE);
			// 座標・回転・スケールを教える
		...
		if (objData.EqualObjectName(L"Search") == true) {
			...
			// 自身の属性を教える
			enemySearch->SetEnemyType(Enemy::TYPE_SEARCH);
			// 座標・回転・スケールを教える
		...
		if (objData.ForwardMatchName(L"Clear") == true) {
			...
			// 自身の属性を教える
			enemyClear->SetEnemyType(Enemy::TYPE_CLEAR);
			// 座標・回転・スケールを教える
