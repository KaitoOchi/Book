...
void Player3D::Update() 
{
	...
	if (g_pad[0]->IsTrigger(enButtonRB1)&&fabsf(m_moveSpeed.y)<0.001)
	{
		Throw();
	}
	//プレイヤーの移動を継承する。
...
void Player3D::Animation()
{
	...
	case Player::m_enPlayer_Jump:
		m_modelRender->PlayAnimation(m_enAnimationClip_Jump, 0.0f);
		break;
	...
	case Player::m_enPlayer3D_Throw:
		m_modelRender->PlayAnimation(m_enAnimationClip_Throw, 0.1f);
		break;
	default:
		break;
	}
